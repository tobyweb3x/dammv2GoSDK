...
func (ac *PgAccounts[T]) All(
		...
		Filters: []rpc.RPCFilter{
			{
				Memcmp: &rpc.RPCFilterMemcmp{
					Offset: 0,
					// Bytes:  base58.Decode(apppendData.String()),
					// Bytes: apppendData.Bytes(),
	...
	if len(filtersParam) > 0 {
		newFilter := slices.Grow(opts.Filters, len(filtersParam))
		newFilter = append(newFilter, filtersParam...)
		opts.Filters = newFilter
	}
